...
- **`__init__.py`** re-exports all dataset builders so you can import them directly from `src.datasets`.
...
- **`__init__.py`** exposes `run_experiment`, `run_rl_experiment`, `RewardHackingPipeline`, `EncodedReasoningPipeline`.
...
- **`__init__.py`** re-exports `probe_encoded_reasoning`, `SAEAnalyzer`, `FeatureAblation`, `CausalAnalyzer`.
...
## Simple Architecture Diagram (Textual)
